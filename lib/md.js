...
function escapeXMLAttribute(value) {
    ...
    // Fix potentially malformed entities
    value = value
        .replace(/&lt([^;])/g, '&lt;$1')
        .replace(/&gt([^;])/g, '&gt;$1')
        .replace(/&amp([^;])/g, '&amp;$1')
...
exports.compileRss = function(posts) {
    ...
    for (var i = 0; i < config.page_count; i++) {
        ...
        rssContent += '    <item>\n';
        rssContent += '      <title>' + escapeXMLAttribute(post.title) + '</title>\n';
        rssContent += '      <description>' + escapeXMLAttribute((post.content).slice(0, 1000)) + '</description>\n';
        rssContent += '      <link>https://blog.bcmeng.com/post/' + post.url + '.html</link>\n';
